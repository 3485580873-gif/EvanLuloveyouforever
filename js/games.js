...
function renderFavorites() {
    ...
    list.innerHTML = favoritedMessages.map(msg => {
        ...
        const content = msg.text
            ...
            : (msg.sticker ? `<img src="${msg.sticker}" style="max-width:100px;max-height:100px;border-radius:8px;display:block;margin-top:4px;">`
            : (msg.voice ? `<div style="display:flex;align-items:center;gap:6px;color:var(--text-secondary);"><i class="fas fa-microphone"></i><span>语音 ${msg.voice.duration || 0}″</span>${msg.voice.fakeText ? `<span style="color:var(--text-primary);">${String(msg.voice.fakeText).replace(/</g,'&lt;').replace(/>/g,'&gt;')}</span>` : ''}</div>`
            : (msg.type === 'red-packet' ? `<div style="color:var(--text-secondary);"><i class="fas fa-money-bill-wave"></i> 转账消息</div>`
            : (msg.type === 'dq-card' ? `<div style="color:var(--text-secondary);"><i class="fas fa-clipboard-list"></i> 梦向问卷</div>`
            : '')))));
