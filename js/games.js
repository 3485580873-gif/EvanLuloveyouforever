...
function initComboMenu() {
    ...
    function renderUserPokeMenu() {
        ...
        // 优先用 customPokes，没有再用默认预设
        const pokeList = (Array.isArray(customPokes) && customPokes.length > 0)
            ? customPokes.slice(0, 8)
            : defaultPresets;
        ...
        title.innerText = (Array.isArray(customPokes) && customPokes.length > 0) ? '我的快捷拍一拍' : '快捷动作';
        wrapper.appendChild(title);

        pokeList.forEach(text => {
