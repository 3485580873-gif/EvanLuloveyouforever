...
if (target.classList.contains('delete-btn')) {
    ...
    return;
            ...
            // 触屏设备上用"轻点"而不是":hover"来展开回复/收藏/删除这几个按钮，
            // 避免滑动聊天记录时手指一碰到气泡就被误当成"悬停"触发出来，容易误触。
            // 用 touchstart/touchend 的移动距离和耗时来判断这次触摸到底是"点一下"还是"在滑动"。
            (function () {
                let touchStartX = 0, touchStartY = 0, touchStartTime = 0, touchTarget = null;
                const TAP_MOVE_THRESHOLD = 8;   // 像素，超过这个距离就认为是在滑动，不是点击
                const TAP_TIME_THRESHOLD = 500; // 毫秒，按太久也不算轻点（可能是长按选择文字等）
                ...
                DOMElements.chatContainer.addEventListener('touchend', (e) => {
                    ...
                    // 点在按钮/图片/语音条这些本身就有点击行为的元素上，不要再额外弹出操作条
                    if (e.target.closest('.meta-action-btn, .message-image, .message-sticker, .voice-bubble, .voice-fake-text, a, button')) return;
                    ...
                    if (dx > TAP_MOVE_THRESHOLD || dy > TAP_MOVE_THRESHOLD || dt > TAP_TIME_THRESHOLD) return; // 是在滑动/长按，不处理
