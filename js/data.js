(function () {
    ...
    function syncToggles() {
        ...
        var isIosPwa = /iphone|ipad|ipod/i.test(ua) && !!window.navigator.standalone;
        if (isIosPwa) {
            n.checked = enabled;
        } else {
        ...
        if (compressBtn && compressInfo) {
            ...
            function getAllStickers() {
                ...
                if (typeof customEmojis !== 'undefined' && Array.isArray(customEmojis)) {
                    customEmojis.forEach(function (s, i) { all.push({ src: s, type: 'customEmojis', index: i }); });
                }
                return all;
            }
            ...
            compressBtn.addEventListener('click', async function () {
                ...
                for (var i = 0; i < all.length; i++) {
                    ...
                    try {
                        ...
                        } else if (item.type === 'customEmojis' && typeof customEmojis !== 'undefined') {
                            customEmojis[item.index] = compressed;
                        }
...
// 检测是否为iOS PWA模式（添加到主屏幕后运行）
var _isIosPwa = (function() {
    var ua = navigator.userAgent || '';
...
window.handleNotifToggle = function(checkbox) {
    ...
    // iOS PWA 模式：系统通知API不可用，但保留开关的UI状态（仅做保活类提醒用）
    if (_isIosPwa) {
        if (checkbox.checked) {
            localStorage.setItem('notifEnabled', '1');
            if (statusEl) statusEl.textContent = '✅ 已开启 — 后台保活时将尝试弹出提醒';
        } else {
...
window._syncNotifToggle = function() {
    ...
    // iOS PWA模式下：不检查Notification.permission（该环境不支持），只看localStorage
    if (_isIosPwa) {
        toggle.checked = enabled;
        if (!statusEl) return;
        if (enabled) {
            statusEl.textContent = '✅ 已开启 — 后台保活时将尝试弹出提醒';
        } else {
