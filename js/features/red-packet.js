    ...
    function showRedPacketSendForm() {
        ...
        submitBtn.onclick = function () {
            ...
            setTimeout(function () {
                ...
                // 已读之后，像发普通消息一样触发对方的自动回复（打字中提示 + 文字回复），
                // 之前转账只会标已读，不会像正常聊天那样收到对方回话
                if (typeof window.simulateReply === 'function') {
                    var replyDelay = delayMin + Math.random() * (delayMax - delayMin);
                ...
                if (Math.random() < 0.2) {
                    ...
                    setTimeout(function () {
                        // 用转账卡片样式展示退款，而不是一条纯文字系统消息，
                        // 这样这笔转账在聊天记录里会有一张完整的"已被退还"卡片
                        if (typeof addMessage === 'function') {
                            addMessage({
                                id: 'rp_ret_card_' + Date.now(),
                                sender: 'partner',
    ...
    window.showRedPacketReceiveModal = function (recordId) {
        ...
        if (isPending && isSystemSender) {
            ...
            if (returnBtn) {
                returnBtn.onclick = function() {
                    ...
                    // 补一张"已退回"的转账卡片，之前这里只改状态、不留任何聊天记录，
                    // 这笔转账看起来就像凭空消失了
                    if (typeof addMessage === 'function') {
                        addMessage({
                            id: 'rp_ret_card_' + Date.now(),
        ...
        if (openBtn && isPending) {
            openBtn.onclick = function() {
                ...
                if (typeof renderMessages === 'function') renderMessages();
                // 添加我方已领取回执卡片
                // 注意：msg.status 是消息已读状态（sent/read），不是转账状态；
                // 转账状态统一存在 msg.redPacket.status 里，避免与已读回执逻辑冲突
                if (typeof addMessage === 'function') {
                    addMessage({
                        id: 'rp_recv_card_' + Date.now(),
                        sender: 'user',
                ...
                // 领取之后，像发普通消息一样触发对方的已读 + 自动回复
                // 1) 先标记已读（red-packet 类型不走 normal 消息的自动已读，手动补上）
                var _readDelay = 1500 + Math.random() * 2500;
                setTimeout(function () {
                    if (typeof messages !== 'undefined' && Array.isArray(messages)) {
                        var _changed = false;
                ...
                // 2) 再触发自动回复（打字中 + 回复消息）
                if (typeof window.simulateReply === 'function') {
                    var _delayMin = (typeof settings !== 'undefined' && settings.replyDelayMin) ? settings.replyDelayMin : 3000;
                    var _delayMax = (typeof settings !== 'undefined' && settings.replyDelayMax) ? settings.replyDelayMax : 7000;
                    var _replyDelay = _delayMin + Math.random() * (_delayMax - _delayMin);
                    // 显示"正在输入"
                    if (typeof settings !== 'undefined' && settings.typingIndicatorEnabled) {
                        var _tiWrapper = document.getElementById('typing-indicator-wrapper');
                        var _tiLabel = document.getElementById('typing-indicator-label');
                        var _tiAvatar = document.getElementById('typing-indicator-avatar');
                        if (_tiLabel) _tiLabel.textContent = (settings.partnerName || '对方') + ' 正在输入';
                        if (_tiWrapper && typeof window.positionTypingIndicator === 'function') {
                            window.positionTypingIndicator();
                            _tiWrapper.style.display = 'block';
                        }
                        if (_tiAvatar && typeof DOMElements !== 'undefined' && DOMElements.partner) {
                            var _partnerImg = DOMElements.partner.avatar.querySelector('img');
                            _tiAvatar.innerHTML = _partnerImg ? '<img src="' + _partnerImg.src + '">' : '<i class="fas fa-user"></i>';
                        }
