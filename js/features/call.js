...
<div id="call-mini-pill">
  ...
  <button class="call-mini-hangup" id="call-mini-hangup">${SVG_HU}</button>
    ...
    // 对方主动挂断的具体执行逻辑：从随机定时器触发，也可以被 /测试对方挂断 手动立即触发
    ...
    // 测试用：跳过概率判定和延迟，立即执行一次"对方主动挂断"，方便验证效果是否正常
    window._triggerPartnerHangupTest = function() {
        if (!S.active) {
            if (typeof showNotification === 'function') showNotification('当前没有进行中的通话，无法测试挂断', 'warning');
            return;
        }
        clearTimeout(S.partnerHangupTimer);
        doPartnerHangup();
