  ...
  <button class="call-mini-hangup" id="call-mini-hangup">${SVG_HU}</button>
    ...
    function showCallRecoverDialog(session) {
        ...
        box.innerHTML = `
            ...
            <div style="font-size:13px;color:var(--text-secondary,#888);line-height:1.6;margin-bottom:18px;">
        ...
        if (hangupBtn) {
            hangupBtn.addEventListener('click', function (e) {
                e.preventDefault(); e.stopPropagation();
                // 之前这里只是把状态清掉、弹个提示，聊天记录里完全没有留下"这通电话挂断了"的痕迹，
                // 看起来就像这通电话凭空消失了。这里补上一条跟正常挂断一样的通话记录。
    ...
    function checkAndResumeCall() {
        ...
        if (!session || !session.active || !session.startTime) { clearCallSession(); return; }
        ...
        if (Date.now() - (session.savedAt || 0) > 30 * 60 * 1000) {
