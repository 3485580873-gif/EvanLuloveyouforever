...
async function applyCustomFont(url) {
    ...
    if (!url || !url.trim()) {
        // 卸载之前注册的自定义字体，防止同名字体残留拦截新字体
        try {
            const old = Array.from(document.fonts).filter(f => f.family === fontName);
            old.forEach(f => { try { document.fonts.delete(f); } catch (e) {} });
        } catch (e) {}
        document.documentElement.style.removeProperty('--font-family');
        document.documentElement.style.removeProperty('--message-font-family');
        return;
    }
    // 每次应用前先卸载同名旧字体：document.fonts 里同名字体堆叠后，
    // CSS 匹配到的永远是第一个（可能是加载失败/旧文件），新字体加了也不生效
    try {
        const oldFonts = Array.from(document.fonts).filter(f => f.family === fontName);
        oldFonts.forEach(f => { try { document.fonts.delete(f); } catch (e) {} });
    } catch (e) {}
    const font = new FontFace(fontName, `url("${url.replace(/"/g, '%22')}")`);
    await font.load();
    ...
    try {
        ...
        const categories = [
            ...
            },
            {
                id: 'replies',
                label: '回复 / 拍一拍 / 氛围',
                indexedDBNeedles: ['customReplies', 'customPokes', 'quickPokes', 'customStatuses', 'customMottos', 'customIntros', 'customEmojis', 'customReplyGroups', 'customPokeGroups', 'customStatusGroups'],
