...
/* ===== 后台保活 · 双引擎版 ===== * 解决：切到抖音等音频App后，保活被抢导致消息不及时 * 原理： * 1. 混合音频模式 (audioSession mixWithOthers) —— 不跟抖音抢焦点，各播各的 * 2. <audio> 标签 + MediaSession —— 让系统知道"这是个媒体App"，提高后台优先级 * 3. Web Audio API 无声振荡器 —— 第二道保险，实时音频上下文更抗挂起 * 4. 多事件监听 + 快速巡检 —— 被暂停后秒级恢复 */
(function () {
  ...
  function _setUI(playing) {
    ...
    if (desc) {
      if (!_get())
        desc.textContent = "混合模式 · 后台保活（可与其他App同时运行）";
