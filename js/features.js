...
(function() {
    ...
    function _createAudio() {
        ...
        _audio.addEventListener('play',  function(){ _setUI(true); _updateMediaSession('playing'); });
        _audio.addEventListener('pause', function(){ _setUI(false); _updateMediaSession('paused'); });
        return _audio;
    }

    // ── Media Session：告诉系统"这是一个正在播放媒体的App"──
    // iOS/Android 对"正在播放音视频"的网页会给更多后台存活时间/更低的优先级挂起概率，
    // 但前提是系统得先"知道"你在播放媒体——只是悄悄放一段静音音频，系统未必会当成
    // 正经的媒体播放。注册 Media Session（连锁屏界面上的媒体控制条都会认得它）能显著
    // 提高系统认定"这是个媒体App，不要轻易挂起它"的概率。
    function _updateMediaSession(state) {
        try {
            if (!('mediaSession' in navigator)) return;
            if (!navigator.mediaSession.metadata) {
                navigator.mediaSession.metadata = new MediaMetadata({
                    title: (typeof settings !== 'undefined' && settings.partnerName) ? settings.partnerName : '陪伴中',
                    artist: '正在陪你聊天',
                    album: '传讯'
                });
                navigator.mediaSession.setActionHandler('play', function () { _start(); });
                navigator.mediaSession.setActionHandler('pause', function () {
                    // 用户在锁屏媒体控制条上按了暂停也无所谓，我们还是想继续保活，
                    // 直接无视这个暂停请求、继续播放
                    if (_get()) _start();
                });
            }
    ...
    // 保活音频很容易被系统在后台悄悄暂停（尤其锁屏/切后台一段时间后），
    // 之前只在切回前台那一下检查一次，容易漏掉；
    // 改成每隔几秒巡检一次，只要设置是开启的、但音频不知不觉停了，就立刻尝试重新播放
    setInterval(function(){
        if (_get() && _audio && _audio.paused) {
            _audio.play().catch(function(){});
        }
    }, 4000);
...
// ════════════════════════════════════════════════════════
// 头像拍一拍功能
// 点击对方头像 → 触发拍一拍（内容可在聊天设置里配置）
// ════════════════════════════════════════════════════════
(function () {
    const LS_KEY = 'avatarPokeTexts';
    const DEFAULT_TEXTS = [
        '拍了拍 {partner} 的头',
        '戳了戳 {partner}',
        '轻轻拍了拍 {partner}',
    ...
    function saveAvatarPokeTexts(arr) {
    ...
    function resolve(tpl) {
        const partnerName = (typeof settings !== 'undefined' && settings.partnerName) || document.getElementById('partner-name')?.textContent.trim() || '对方';
        const myName = (typeof settings !== 'undefined' && settings.myName) || '我';
        return tpl.replace(/{partner}/g, partnerName).replace(/{me}/g, myName);
    }

    // 点击头像触发拍一拍
    function handleAvatarPoke(e) {
        e.stopPropagation();
        const texts = getAvatarPokeTexts();
        if (!texts.length) return;
        const tpl = pickRandom(texts);
        const pokeText = resolve(tpl);
        const formatted = (typeof window._formatPokeText === 'function') ? window._formatPokeText(pokeText) : pokeText;
    ...
    function bindAvatarClick() {
        const avContainer = document.getElementById('partner-avatar-container') || document.getElementById('partner-avatar');
        if (!avContainer || avContainer._avatarPokesBound) return;
        avContainer._avatarPokesBound = true;
        avContainer.style.cursor = 'pointer';
        avContainer.title = '拍一拍';
        avContainer.addEventListener('click', handleAvatarPoke);
    }

    // 设置弹窗
    window._openAvatarPokeSettings = function () {
        const texts = getAvatarPokeTexts();
        const overlay = document.createElement('div');
        overlay.id = 'avatar-poke-settings-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;z-index:999999;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.5);backdrop-filter:blur(8px);';

        const box = document.createElement('div');
        box.style.cssText = 'width:min(90vw,380px);background:var(--secondary-bg,#fff);border-radius:18px;padding:22px 20px 18px;box-shadow:0 20px 50px rgba(0,0,0,.3);max-height:80vh;overflow-y:auto;';
        box.innerHTML = `
            <div style="font-size:16px;font-weight:700;color:var(--text-primary);margin-bottom:4px;display:flex;align-items:center;gap:8px;">
                <i class="fas fa-user-circle" style="color:var(--accent-color);"></i> 头像拍一拍
            </div>
            <div style="font-size:12px;color:var(--text-secondary);margin-bottom:14px;">点击对方头像时，会随机触发下方的一条拍一拍内容<br>用 {partner} 代表对方昵称，{me} 代表我的昵称</div>
            <div id="apo-list" style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px;"></div>
            <div style="display:flex;gap:8px;">
                <input type="text" id="apo-new-input" placeholder="添加新的拍一拍内容…" style="flex:1;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;font-size:13px;background:var(--primary-bg);color:var(--text-primary);outline:none;font-family:var(--font-family);">
                <button id="apo-add-btn" style="padding:9px 14px;border:none;border-radius:10px;background:var(--accent-color);color:#fff;font-size:13px;font-weight:600;cursor:pointer;">添加</button>
            </div>
            <div style="display:flex;gap:8px;margin-top:14px;">
                <button id="apo-reset-btn" style="flex:1;padding:10px;border:none;border-radius:12px;background:var(--message-received-bg,#f0f0f0);color:var(--text-secondary);font-size:13px;cursor:pointer;">恢复默认</button>
                <button id="apo-close-btn" style="flex:1;padding:10px;border:none;border-radius:12px;background:var(--accent-color);color:#fff;font-size:14px;font-weight:600;cursor:pointer;">完成</button>
            </div>
        `;
        overlay.appendChild(box);
        document.body.appendChild(overlay);

        let localTexts = texts.slice();

        function renderList() {
            const list = box.querySelector('#apo-list');
            list.innerHTML = '';
            localTexts.forEach((txt, i) => {
                const row = document.createElement('div');
                row.style.cssText = 'display:flex;align-items:center;gap:8px;background:var(--primary-bg);border:1px solid var(--border-color);border-radius:10px;padding:8px 10px;';
                row.innerHTML = `<span style="flex:1;font-size:13px;color:var(--text-primary);">${txt}</span><button data-i="${i}" style="border:none;background:none;cursor:pointer;color:var(--text-secondary);font-size:13px;padding:2px 6px;" title="删除">✕</button>`;
                row.querySelector('button').addEventListener('click', () => {
                    localTexts.splice(i, 1);
                    renderList();
                });
        ...
        box.querySelector('#apo-add-btn').addEventListener('click', () => {
            const inp = box.querySelector('#apo-new-input');
        ...
        box.querySelector('#apo-new-input').addEventListener('keydown', e => {
            if (e.key === 'Enter') box.querySelector('#apo-add-btn').click();
        });
        box.querySelector('#apo-reset-btn').addEventListener('click', () => {
            localTexts = DEFAULT_TEXTS.slice();
            renderList();
        });
        box.querySelector('#apo-close-btn').addEventListener('click', () => {
            saveAvatarPokeTexts(localTexts);
            document.body.removeChild(overlay);
            if (typeof showNotification === 'function') showNotification('头像拍一拍已保存 ✓', 'success', 1800);
        });
        overlay.addEventListener('click', e => { if (e.target === overlay) box.querySelector('#apo-close-btn').click(); });
    };

    // 初始化：等DOM就绪后绑定
    function init() {
        bindAvatarClick();
        // 防止后续动态替换头像容器后失效，监听变化
        const header = document.querySelector('.chat-header, header, #chat-header');
        if (header) {
            new MutationObserver(bindAvatarClick).observe(header, { childList: true, subtree: true });
        }
    }
