    ...
    throttledSaveData();
        ...
        function sendMessage(textOverride = null, type = 'normal') {
            ...
            if (text && text.startsWith('/') && type === 'normal') {
                ...
                if (cmd === '/测试挂断' || cmd === '/testhangup') {
                    DOMElements.messageInput.value = '';
                    DOMElements.messageInput.style.height = '46px';
                    if (typeof window._triggerPartnerHangupTest === 'function') window._triggerPartnerHangupTest();
                    if (typeof showNotification === 'function') showNotification('✦ 强制触发对方中途挂断（需先处于通话中）', 'info', 2200);
                    return;
                }
            }
