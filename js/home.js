...
(function() {
    ...
    function renderIconGrid() {
        ...
        grid.innerHTML = '';
        const nameMap = { chat:'聊天', mailbox:'信封', moyu:'摸鱼', diary:'朝夕心记', fortune:'运势', mood:'心晴', calendar:'日历', decide:'抉择', stats:'统计', accounting:'记账','ta-phone':'TA的手机'};
    ...
    function initPartnerMomentSettings() {
            ...
            toggle.addEventListener('click', function() {
                ...
                if (typeof window.MomentsApp !== 'undefined') {
                    if (newVal) {
                        ...
                        window.MomentsApp.stopPartnerMomentTimer();
                    }
                }
            });
        }
        ...
        if (minSlider) {
            minSlider.addEventListener('input', function() {
                localStorage.setItem('partner_moment_interval_min', this.value);
                if (minVal) minVal.textContent = this.value + '分钟';
            });
        }
        if (maxSlider) {
            maxSlider.addEventListener('input', function() {
                localStorage.setItem('partner_moment_interval_max', this.value);
                if (maxVal) maxVal.textContent = this.value + '分钟';
            });
        }
        if (countSlider) {
            countSlider.addEventListener('input', function() {
                localStorage.setItem('partner_moment_count_max', this.value);
                if (countVal) countVal.textContent = this.value + '条';
            });
        }
    }
    ...
    function initPartnerCardSpliceSettings() {
        ...
        if (probSlider) {
            probSlider.addEventListener('input', function() {
                localStorage.setItem('partner_card_splice_probability', this.value);
                if (probVal) probVal.textContent = this.value + '%';
            });
        }
        if (minSlider) {
            minSlider.addEventListener('input', function() {
                localStorage.setItem('partner_card_splice_min', this.value);
                if (minVal) minVal.textContent = this.value + '条';
            });
        }
        if (maxSlider) {
            maxSlider.addEventListener('input', function() {
                localStorage.setItem('partner_card_splice_max', this.value);
                if (maxVal) maxVal.textContent = this.value + '条';
            });
        }
    }
